...
**Phase 2 — planned, not yet built.** An embedding encrypted against the [enclave](https://github.com/kimjune01/vectorspace-enclave)'s attested key is decrypted and auctioned inside a TEE, so the exchange operator cannot read it. This is not shipped: `GET /tee/attestation` returns a placeholder document, the vsock enclave path is stubbed (Phase 2b), and the default server (`--tee` off) runs a `MockTEEProxy` that decrypts **in the exchange's own process**. So in the default deployment the exchange process does see the embedding. Do not rely on the TEE privacy property until real NSM attestation and vsock land ([roadmap](ROADMAP.md)). The Phase 1 on-device path is the guarantee that holds today.
...
- **TEE attestation is a placeholder** — real NSM attestation + vsock are Phase 2b (see Privacy). Until then the TEE privacy property is aspirational; Phase 1 on-device is what holds.
