...
// Unlike RunAdRequest, it includes ALL advertisers regardless of budget and
// does not log to the database or generate click URLs.
func (e *AuctionEngine) SimulateAuction(intent string) (*SimulationResult, error) {
	queryEmbedding, err := e.Embedder.Embed(intent)
	if err != nil {
		return nil, fmt.Errorf("embed query intent: %w", err)
	}
	...
	// Build bids from ALL positions (no budget filtering)
